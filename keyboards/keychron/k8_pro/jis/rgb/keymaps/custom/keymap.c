...
// Remove custom effect enum as it's defined in rgb_matrix_user.inc
enum layers{
    MAC_BASE,
    MAC_FN,
    WIN_BASE,
...
void keyboard_post_init_user(void) {
    rgb_matrix_enable();
    rgb_matrix_mode(RGB_MATRIX_CUSTOM_FIRE_AND_ICE);
}

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        // Add ripple effect on key press
        add_ripple(record->event.key.row, record->event.key.col);
    }
    return true;
} 